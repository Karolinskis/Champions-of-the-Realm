...
>>>>>>> Stashed changes
	...
	public WeaponsManager WeaponsManager { get; set; } // For handling weapons
...
=======
	...
	public override void _Input(InputEvent @event)
	{
		...
		if (@event is InputEventMouseButton eventMouseButton)
		{
			// handling pressed mouse buttons
			if (eventMouseButton.ButtonIndex == MouseButton.Left && eventMouseButton.IsPressed() && !isDefending)
			{
				float angle = GetGlobalTransformWithCanvas().Origin.AngleToPoint(eventMouseButton.Position);
				if (WeaponsManager.Attack(angle))
				{
			...
			if (eventMouseButton.ButtonIndex == MouseButton.Right && eventMouseButton.IsPressed() && !isDefending)
			{
				Modulate = new Color("74b1ff"); // defending color
				Stats.Armour += 5;
				defendTimer.Start();
				isDefending = true;
			}
		}
		else if (@event is InputEventKey eventKeyboardKey) // handling pressed keys
		{
...
>>>>>>> Stashed changes
	...
	/// <summary>
	/// Method for handling received gold and adding to current player amount
	/// </summary>
	...
	/// <summary>
	/// Method for handling received xp
	/// </summary>
...
=======
	...
	/// <summary>
	/// Metheod for handling attack timer timout
	/// </summary>
	private void AttackTimerTimeout()
	{
		//if (CurrentWeapon is Melee)
